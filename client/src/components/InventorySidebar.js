...
const InventorySidebar = ({
  ...
  // Single add (now uses backend API)
  ...
  // Multi add (batch add to local batch)
  const handleMultiAdd = (e) => {
    ...
    if (editingIdx !== null) {
      setMultiBatch((prev) =>
        prev.map((item, idx) =>
          idx === editingIdx ? { ...form, id: item.id } : item
        )
      );
      setEditingIdx(null);
    } else {
  ...
  // Multi submit (now uses backend API)
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto h-[calc(100vh-64px)]">
          {/* Form */}
          <form
            onSubmit={mode === "multi" ? handleMultiAdd : handleSingleSubmit}
            className={`grid grid-cols-1 gap-4 mb-2 p-4 rounded-xl shadow-md ${
          ...
          >
            ...
            <div className="flex gap-2 justify-end mt-2">
              {mode === "multi" && (
                <button
                  type="submit"
                  className={`px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
                    isDarkMode
              ...
              {mode === "single" && (
                <button
                  type="submit"
                  className={`px-6 py-2 rounded-lg font-semibold shadow transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
                    isDarkMode
              ...
              {mode === "multi" && multiBatch.length > 0 && (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-1 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-400 ${
                    isDarkMode
                      ? "bg-green-700 text-white hover:bg-green-600"
                      : "bg-success-100 text-success-700 hover:bg-success-200"
                  }`}
                >
                  Submit All ({multiBatch.length})
                </button>
              )}
            </div>
          </form>
          ...
          {errorMsg && (
            <div
              className={`mb-4 text-sm font-semibold rounded p-3 ${
                isDarkMode
