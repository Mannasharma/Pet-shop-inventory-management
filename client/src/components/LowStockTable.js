...
const LowStockTable = ({ items, isDarkMode = false }) => {
  const getStockStatus = (current, min) => {
    ...
    if (isDarkMode) {
      if (percentage <= 50)
        return { color: "text-red-400", bg: "bg-red-900/20", text: "Critical" };
      if (percentage <= 75)
        return {
      ...
      if (percentage <= 75)
        return { color: "text-warning-600", bg: "bg-warning-50", text: "Low" };
      return {
            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-all duration-300 ${status.bg} ${status.color}`}
                    >
                      {status.text}
                    </span>
                  </td>
