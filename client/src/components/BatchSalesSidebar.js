...
const BatchSalesSidebar = ({
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => ({
      ...prev,
      [name]: value,
      ...(name === "product_search" ? { pet_food_id: "" } : {}),
    }));
  ...
  const handleAddOrUpdate = (e) => {
    ...
    const inv = inventory.find(
      (i) => String(i.id) === String(form.pet_food_id)
    );
    if (!inv) {
    ...
    if (editingIdx !== null) {
      setBatch((prev) =>
        prev.map((row, i) => (i === editingIdx ? { ...form } : row))
      );
      setEditingIdx(null);
    } else {
  ...
  const handleSubmitAll = async () => {
    ...
    for (const row of batch) {
      ...
      const inv = inventory.find(
        (i) => String(i.id) === String(row.pet_food_id)
      );
      if (!inv) {
        setError("Invalid product selected.");
        return;
    ...
    try {
      ...
      console.error("Failed to submit batch sales:", error);
  ...
  return (
    ...
    >
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md shadow-2xl transition-transform duration-300 z-50 ${
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-inherit backdrop-blur-md rounded-t-2xl">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-red-100 dark:hover:bg-red-900/40 focus:outline-none focus:ring-2 focus:ring-red-400 transition-colors shadow-md"
          >
            <X size={24} />
          </button>
        </div>
        <div className="p-6 overflow-y-auto h-[calc(100vh-80px)] flex flex-col gap-4">
          ...
          <form
            ...
            className={`w-full rounded-xl shadow-lg p-5 mb-4 ${
              isDarkMode ? "bg-gray-800/90" : "bg-gray-100"
            }`}
          >
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  readOnly
                  disabled
                  className={`rounded-lg px-4 py-3 text-base border w-full transition-colors duration-300 bg-gray-200 text-gray-500 cursor-not-allowed ${
                    isDarkMode
              ...
              <div className="flex gap-3 mt-2">
                <button
                  type="submit"
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg py-3 font-semibold flex items-center justify-center gap-2"
                  disabled={!form.pet_food_id || isInventoryEmpty}
                >
                  {editingIdx !== null ? (
                    <Edit2 size={18} />
                  ) : (
                ...
                {batch.length > 0 && (
                  <button
                    ...
                    className="flex-1 bg-green-700 hover:bg-green-800 text-white rounded-lg py-3 font-semibold flex items-center justify-center gap-2"
                    disabled={isInventoryEmpty}
                  >
              ...
              {error && (
                <div className="text-red-500 text-base mt-2">{error}</div>
              )}
            </div>
            {isInventoryEmpty && (
              <div
                className={`mt-4 text-center text-base font-semibold rounded p-3 ${
                  isDarkMode
                    ? "bg-yellow-900 text-yellow-200"
                    : "bg-yellow-100 text-yellow-700"
                }`}
              >
                Please add at least 1 product in the inventory.
              </div>
            )}
          </form>
          {/* Divider */}
          ...
          <div>
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto">
              {batch.map((item, idx) => {
                const inv = inventory.find(
                  (i) => i.id === Number(item.pet_food_id)
                );
                return (
                  <div
                    key={idx}
                    ...
                    } rounded-lg p-4 flex flex-col shadow hover:shadow-lg transition`}
                  >
                    <div className="flex justify-between items-center">
                      ...
                      >
                        {inv ? inv.name : item.product_search}
                      </span>
                      <div className="flex gap-2">
                        <button
                    ...
                    >
                      {inv ? inv.brand : ""} | {inv ? inv.category : ""} | ₹
                      {item.revenue} | {item.quantity_sold}{" "}
                      {inv ? inv.unit : ""} | Exp:{" "}
                      {item.expiry || inv?.expiry || "-"}
                    </div>
                    {inv && (item.expiry || inv.expiry) && (
                      <div className="text-xs mt-1 font-medium text-black">
                        Brand {inv.brand}, 1 product, expires on{" "}
                        {item.expiry || inv.expiry}
                      </div>
                    )}
                  </div>
